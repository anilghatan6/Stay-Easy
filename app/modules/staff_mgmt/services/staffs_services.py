...
from app.utils.exceptions import ServiceException, RepositoryException,StaffAlreadlyExistException, StaffNotFound, PropertyNotFoundException, ImageStorageException

from app.modules.staff_mgmt.schemas.staffs_schemas import CreateStaffRequest,UpdateStaffRequest,StaffResponse
...
class StaffService:
    ...
    async def create_staff(
        ...
        try:
            ...
            if existing is not None:
                raise StaffAlreadlyExistException(f"A staff member with email {staff_data["email"]} already exists")
        ...
        except (StaffAlreadlyExistException, PropertyNotFoundException, RepositoryException) as e:
    ...
    async def get_staff_by_id(self, tenant_id:uuid.UUID,property_id:uuid.UUID,staff_id: uuid.UUID) -> StaffResponse:
        try:
    ...
    async def list_staff(self, tenant_id: uuid.UUID, skip: int = 0, limit: int = 10) -> list[StaffResponse]:
        try:
            staff_list, total = await self.staff_repo.list_by_tenant(tenant_id, skip, limit)
    ...
    async def list_staff_by_property(self,tenant_id:uuid.UUID, property_id: uuid.UUID, skip: int = 0, limit: int = 10) -> List[StaffResponse]:
        try:
            property_obj = await self.prop_repo.get_property_by_id(property_id,tenant_id)
            if property_obj is None:
                raise PropertyNotFoundException("Property not found")

            staff_list, total = await self.staff_repo.list_by_property(property_id,skip,limit)
            ...
            logger.error(f"[StaffService] Error listing staff by property: {e}", exc_info=True)
    ...
    async def update_staff(
        ...
        try:
            ...
            if payload.email and payload.email != staff.email:
                ...
                if email_owner is not None and email_owner.id != staff_id:
                    raise StaffAlreadlyExistException(f"A staff member with email {payload.email} already exists")
            ...
            if "photos" in update_data and update_data["photos"]:
                ...
                active_photos_to_promote = {k: v for k, v in merged_photos.items() if v is not None}
                promoted_photos = await self._promote_staff_images_if_any(active_photos_to_promote, staff_id)
        ...
        except (PropertyNotFoundException, StaffNotFound, StaffAlreadlyExistException, RepositoryException):
            await self.db.rollback()
            raise
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating staff", exc_info=True)
            raise ServiceException("Error updating staff")

    async def delete_staff(self, tenant_id:uuid.UUID, property_id:uuid.UUID, staff_id: uuid.UUID) -> None:
        try:
            property_obj = await self.prop_repo.get_property_by_id(property_id,tenant_id)
            if property_obj is None:
                raise PropertyNotFoundException("Property not found")
