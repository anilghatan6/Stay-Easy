...
class RoomUpdate(BaseModel):
    room_name: Optional[str] = Field(None, min_length=1, max_length=100, examples=["Ocean Suite A"])
        ...
        if policy is not None:
            ...
            if policy == CancellationPolicy.CUSTOM or policy == "CUSTOM":
                if not title or not str(title).strip():
                    raise ValueError("cancellation title is required when cancellation policy is CUSTOM.")
                if not description or not str(description).strip():
                    raise ValueError("cancellation description is required when cancellation policy is CUSTOM.")
