...
def validate_khalti_return_url(url: str) -> str:
    ...
    """
    base = (os.getenv("KHALTI_RETURN_URL_BASE") or os.getenv("KHALTI_RETURN_URL") or "").rstrip("/")

    if not base:
        logger.error("[URLValidation] Neither KHALTI_RETURN_URL_BASE nor KHALTI_RETURN_URL is configured in environment")
        raise ValueError("KHALTI_RETURN_URL is not configured on the server")

    if not url or not url.startswith(base):
        logger.warning(f"[URLValidation] Rejected return_url not matching base '{base}': {url}")
        raise ValueError(f"return_url must start with '{base}'")
